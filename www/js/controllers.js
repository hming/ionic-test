...
.controller('TutorialCtrl', function($scope, $state, $ionicHistory) {
	...
	window.localStorage['didTutorial'] = true; // For Test
...
// 录音页面
// *******************
.controller('RecordCtrl', function($scope, $state, $stateParams, $cordovaMedia, $timeout) {
	$scope.c = 0;
	...
	function r(src) {
		var my_media = new Media(src,
			...
			function(code) {
				//if (code == Media.MEDIA_RUNNING) {
//					$scope.media.getCurrentPosition(function(p){
//						$scope.playTime = p;
//					});
				//}
			}
	...
	function timedCount() {
		var h = parseInt($scope.c / 3600);
		var m = parseInt($scope.c / 60);
		var s = $scope.c % 60;
		$scope.time = intFormat(h, 2) + ':' + intFormat(m, 2) + ':' + intFormat(s, 2);
		$scope.c++;
		$scope.timer = $timeout(timedCount, 1000);
	};
	...
	function timedPlay() {
		...
		$scope.playTimer = $timeout(timedPlay, 10);
	};
...
function dateFormat(date, str) {
	...
	return str.replace(/yyyy/g, date.getFullYear()).
	replace(/MM/g, intFormat(date.getMonth() + 1, 2)).
	replace(/dd/g, intFormat(date.getDate(), 2)).
	replace(/HH/g, intFormat(date.getHours(), 2)).
	replace(/mm/g, intFormat(date.getMinutes(), 2)).
	replace(/ss/g, intFormat(date.getSeconds(), 2)).
	replace(/mi/g, intFormat(date.getMilliseconds(), 2));
}

function intFormat(i, n) {
	var rt = i.toString()
